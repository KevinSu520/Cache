...
void cachesim_init(int _block_size, int _cache_size, int _ways) {
    ...
    ways = _ways;
    num_sets = cache_size / (block_size * ways); // find the number of sets
    ...
    cache = (cache_set_t *)malloc(num_sets * sizeof(cache_set_t)); // allocate memory for cache

    for (int i = 0; i < num_sets; i++) {
        cache[i].size = 0; //initialize the cache set
        cache[i].stack = init_lru_stack(ways); //initialize LRU stack
        cache[i].blocks = (cache_block_t *)malloc(ways * sizeof(cache_block_t)); // initialize cache block structs for the set

        for(int j = 0; j < ways; j++) { 
            cache[i].blocks[j].tag = 0; //initialize the current tag bit
            cache[i].blocks[j].valid = 0; //initialize the current valid bit
            cache[i].blocks[j].dirty = 0; //initialize the current dirty bit
        }
...
void cachesim_access(addr_t physical_addr, int access_type) {
    ...
    accesses++; //increment the number of accesses counter
    int not_found = 1; //boolean for not be able to determine miss or hit
    int mask = 0xFFFFFFFF; //mask to get the tag bits
    
    addr_t index = ((~(mask << num_index_bits) << num_offset_bits) & physical_addr) >> num_offset_bits; //get the index bits from address
    addr_t tag = ((mask << (num_offset_bits + num_index_bits)) & physical_addr) >> (num_offset_bits + num_index_bits); //get the tag bits from address

    
    for (int i = 0; i < ways; i++) //search cache blocks to see if hit or miss
    {
        if (cache[index].blocks[i].valid && cache[index].blocks[i].tag == tag) //condition to check if cache is valid and tag is correct
        {
            not_found = 0; //set boolean to false

            hits++; // increment hits counter

            if (access_type == MEMWRITE)
            {
                cache[index].blocks[i].dirty = 1; //if memory write, set dirty bit to 1
            }
            
            lru_stack_set_mru(cache[index].stack, i); //now we move this index to MRU
            
            break;
        } 
    else if (!cache[index].blocks[i].valid && cache[index].blocks[i].tag != tag) //if the cache block is not valid and tag not equal, we have a miss
     {
            not_found = 0; //set boolean to false
            misses++; //increment misses counter
            
            //bring in the new cache block
            cache[index].blocks[i].valid = 1; //set valid bit to 1
            cache[index].blocks[i].tag = tag; //set tag bit to tag
            cache[index].size++; //increment the size of the cache set
            ...
            if (access_type == MEMWRITE)
            {
                ...
                cache[index].blocks[i].dirty = 0; //if memory read, set dirty bit to 0
            }
             
             lru_stack_set_mru(cache[index].stack, i); //move this index to MRU
            
            break;
        }
    }
     
     //if we have a miss and the cache set is full, we need to evict the LRU block
     if(not_found) {
        misses++; //increment misses counter
        int lru_index = lru_stack_get_lru(cache[index].stack); //get the LRU index

        cache[index].blocks[lru_index].tag = tag; //set the tag bit to tag
        cache[index].blocks[lru_index].valid = 1; //set the valid bit to 1

        if (cache[index].blocks[lru_index].dirty) //if the dirty bit is 1, we need to write back
        {
            writebacks++; //increment writebacks counter
        }
        if(access_type == MEMWRITE) {
            cache[index].blocks[lru_index].dirty = 1; //if memory write, set dirty bit to 1
        }
        else {
            cache[index].blocks[lru_index].dirty = 0; //if memory read, set dirty bit to 0
        }
        lru_stack_set_mru(cache[index].stack, lru_index); //move this index to MRU
...
void cachesim_cleanup() {
    ...
    for (int i = 0; i < num_sets; i++) { //deallocate the blocks and the stacks.
        free(cache[i].blocks);
        lru_stack_cleanup(cache[i].stack);
    }

    free(cache); //free the caches;
